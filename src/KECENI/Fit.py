...
class Fit:
    ...
    def smoothed_G_estimate(self, i0, T0, G0=None, 
                    n_sample=1000, lamdas=1, n_process=1, 
    ...
    def m_D(self, j, T_N1k, Xs_N2k, G_N2k, lamdas=1, n_sample=100):
        ...
        Ws_N2j = np.exp(- lamdas[...,None,None] 
                        # * Ds_N2j)
    ...
    def loo_cv_k(self, k, lamdas, n_sample=100, n_process=1, 
        ...
        return fit_mk.DR_estimate(
            k, self.data.Ts, self.data.G, n_sample=n_sample, 
            lamdas=lamdas, n_process=n_process, 
            tqdm = tqdm, leave_tqdm = leave_tqdm
        )

    def loo_cv_old(self, lamdas, n_cv=100, n_sample=100, n_process=1, 
                   tqdm=None, leave_tqdm=True):
        if tqdm is None:
            def tqdm(iterable, *args, **kwargs):
                return iterable
