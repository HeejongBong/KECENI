...
class Fit:
    ...
    def AIPW_j(self, j, i0s, T0s=None, G0=None, n_X=100, seed=12345):
        ...
        return (
            Ds_N2j.reshape(ITb.b.shape),
            mus_N2j[0], pis_N2j[0], np.mean(mus_N2j[1:]), np.mean(pis_N2j[1:])
        )

    def kernel_AIPW(self, i0s, T0s=None, G0=None, js=None,
                    n_X=100, n_process=1, tqdm=None, level_tqdm=0):
        if G0 is None:
            G0 = self.data.G
            if T0s is None:
                T0s = self.data.Ts
        ...
        Ds, mus, pis, ms, vps = list(zip(*r))
        
        return KernelEstimate(self, i0s, T0s, G0, n_X, js, np.array(Ds), 
                              np.array(mus), np.array(pis), np.array(ms), np.array(vps))
