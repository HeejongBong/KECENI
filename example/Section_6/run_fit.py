...
def run_task(task_id):    
    ...
    result_fit = keceni_model.fit(data, n_X=n_X, js=js, tqdm=tqdm)
    
    np.savez('result_fit/%.3d.npz'%(task_id), 
             n_X=result_fit.n_X, js=result_fit.js,
             mus=result_fit.mus, pis=result_fit.pis, 
             ms=result_fit.ms, vps=result_fit.vps)
